...
exports.charts = asyncHandler(async (_req, res) => {
  ...
  const revenueTrend = (await query(
    `SELECT to_char(date_trunc('month',created_at),'Mon') month,
            coalesce(sum(amount),0)::float total
     FROM bookings GROUP BY 1, date_trunc('month',created_at) ORDER BY date_trunc('month',created_at)`)).rows;
